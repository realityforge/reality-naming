...
module Reality
  module Naming
    ...
    def self.pascal_case(input_word)
      word_parts = split_into_words(input_word).collect{|part| part[0...1].upcase + part[1..-1]}
    ...
    def self.underscore(input_word)
      word = split_into_words(input_word).join('_')
      word.downcase!
      word
    end

    def self.xmlize?(word)
      xmlize(word) == word
    end
    ...
    def self.uppercase_constantize?(word)
